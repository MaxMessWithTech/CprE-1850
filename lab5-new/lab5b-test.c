...
void print_mags(Vector3D vec, enum FallState fs);
...
int main(void) {
	...
	enum FallState fs = STILL;

	// fflush(stdout);
	// printf("Maxwell Miller\n");
	// printf("mill06\n");
	printf("Time (ms),UNKNOWN,STILL,FALLING,STOPPED\n");

	// Won't release until it has gotten the first input
	accel = getInput();
	print_mags(tempAccel, fs);
	...
	// While not still, keep printing
	while (1) {
		tempAccel = getInput();
		
		if (tempAccel.t == accel.t) {
			break;
		}
